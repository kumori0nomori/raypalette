...
void draw_hsv_point_cloud(const HsvPlotCanvas &canvas,
                          const std::vector<HsvPlotPoint> &points) {
  ...
  for (const HsvPlotPoint &point : points) {
    projected_points.push_back({canvas.project(point.cylinder_position),
                                point.display_color});
  }
...
void draw_hsv_marker(const HsvPlotCanvas &canvas, const char *label,
                     const raypalette::Vec3 &color, bool diamond, float radius) {
  const HsvScreenPoint screen = canvas.project(
  ...
  if (diamond) {
    canvas.draw_list->AddQuadFilled(
    ...
    canvas.draw_list->AddQuad(
      ImVec2(screen.position.x, screen.position.y - radius),
      ImVec2(screen.position.x + radius, screen.position.y),
      ImVec2(screen.position.x, screen.position.y + radius),
      ImVec2(screen.position.x - radius, screen.position.y), outline_color, 1.5f);
  } else {
  ...
  canvas.draw_list->AddText(ImVec2(screen.position.x + radius + 3.0f,
                                   screen.position.y - 7.0f),
                              outline_color, label);
}

void draw_hsv_reference_markers(const HsvPlotCanvas &canvas,
                                const raypalette::Vec3 &sphere_color,
                                const raypalette::Vec3 &light_color_value,
                                float light_energy_value,
  ...
  const float normalized_energy = light_energy_maximum > 0.0f
      ...
      : 0.0f;
  draw_hsv_marker(canvas, "Light", light_color_value, false,
                  5.0f + 6.0f * normalized_energy);
  for (int index = 0; index < static_cast<int>(palette.size()); ++index) {
    const HsvScreenPoint screen = canvas.project(
      raypalette::hsv_cylinder_position(
        raypalette::srgb_to_hsv(palette[index].color)));
    const float radius = selected_palette_index == index ? 7.0f : 5.0f;
    const ImU32 outline_color = ImGui::GetColorU32(ImGuiCol_Text);
    canvas.draw_list->AddRectFilled(
...
void select_hue_at_mouse(const HsvPlotCanvas &canvas, HsvHueSection &section) {
  ...
  for (int sample = 0; sample < kHueSelectionSamples; ++sample) {
    ...
    const HsvScreenPoint rim = canvas.project(
      raypalette::hsv_cylinder_position({hue, 1.0f, 0.0f}));
...
void draw_hsv_cylinder_guides(const HsvPlotCanvas &canvas, ImU32 guide_color) {
  ...
  for (int segment = 0; segment < kRingSegments; ++segment) {
    ...
    canvas.draw_list->AddLine(lower_a_screen.position, lower_b_screen.position,
                              guide_color, 1.0f);
    canvas.draw_list->AddLine(upper_a_screen.position, upper_b_screen.position,
                              guide_color, 1.0f);
    if (segment % 4 == 0) {
      canvas.draw_list->AddLine(lower_a_screen.position, upper_a_screen.position,
                                guide_color, 1.0f);
    }
  }
}

  raypalette::Vec3 cylinder_position_at_hue(float hue, float saturation,
                        float value) {
    constexpr float kTwoPi = 6.28318530717958647692f;
  ...
  void draw_hsv_section_boundary(const HsvPlotCanvas &canvas, float hue,
                   ImU32 outline_color) {
    const HsvScreenPoint lower = canvas.project(
      cylinder_position_at_hue(hue, 1.0f, 0.0f));
    const HsvScreenPoint upper = canvas.project(
      cylinder_position_at_hue(hue, 1.0f, 1.0f));
  ...
  void draw_hsv_section_plane(const HsvPlotCanvas &canvas,
                const HsvHueSection &section) {
    const HsvScreenPoint bottom_center = canvas.project({0.0f, 0.0f, 0.0f});
    const HsvScreenPoint bottom_edge = canvas.project(
      cylinder_position_at_hue(section.hue, 1.0f, 0.0f));
    const HsvScreenPoint top_edge = canvas.project(
      cylinder_position_at_hue(section.hue, 1.0f, 1.0f));
    ...
    const ImU32 outline_color = ImGui::GetColorU32(ImGuiCol_Text);
    canvas.draw_list->AddQuadFilled(bottom_center.position, bottom_edge.position,
                    top_edge.position, top_center.position,
                    section_color);
    canvas.draw_list->AddLine(bottom_center.position, bottom_edge.position,
                outline_color, 2.0f);
    canvas.draw_list->AddLine(bottom_edge.position, top_edge.position,
                outline_color, 2.0f);
    canvas.draw_list->AddLine(top_edge.position, top_center.position,
                outline_color, 2.0f);
    draw_hsv_section_boundary(
      canvas, std::fmod(section.hue - section.half_width + 1.0f, 1.0f),
      outline_color);
    draw_hsv_section_boundary(
      canvas, std::fmod(section.hue + section.half_width, 1.0f), outline_color);
  }
...
void draw_hsv_space(const std::vector<HsvPlotPoint> &points,
  ...
  ImGui::InvisibleButton("##hsv-space-canvas",
                         ImVec2(canvas_size, canvas_size),
  ...
  draw_hsv_reference_markers(canvas, sphere_color, light_color_value,
                             light_energy_value, light_energy_maximum,
