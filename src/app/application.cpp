...
std::vector<HsvPlotPoint> make_sphere_hsv_points(const Texture& texture,
  ...
  for (int pixel_y = stride / 2; pixel_y < texture.height; pixel_y += stride) {
    for (int pixel_x = stride / 2; pixel_x < texture.width; pixel_x += stride) {
      ...
      if (!raypalette::hit_sphere(scene.sphere, raypalette::camera_ray(camera, u, v),
      ...
      points.push_back({display_color, hsv, raypalette::hsv_cylinder_position(hsv)});
    }
  }
  return points;
}
...
void select_hue_at_mouse(const HsvPlotCanvas& canvas, HsvHueSection& section) {
  ...
  for (int sample = 0; sample < kHueSelectionSamples; ++sample) {
    ...
    const HsvScreenPoint rim = canvas.project(raypalette::hsv_cylinder_position({hue, 1.0f, 0.0f}));
  ...
  for (int value_step = 0; value_step < kValueSteps; ++value_step) {
    ...
    for (int saturation_step = 0; saturation_step < kSaturationSteps; ++saturation_step) {
      ...
      draw_list->AddRectFilled(
          ...
          hsv_plot_color(raypalette::hsv_to_srgb({section.hue, saturation, value})));
    }
  }
