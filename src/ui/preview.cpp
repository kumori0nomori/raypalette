  ...
  if (context.texture.id != 0) {
    ...
    if (ImGui::IsItemHovered() && ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
      ...
      if (pixel_x >= 0 && pixel_x < context.image.width && pixel_y >= 0 &&
        ...
        if (context.palette.size() < kMaximumPaletteColors) {
          bool already_added = false;
          for (const PaletteColor& entry : context.palette) {
            already_added = already_added || same_palette_color(entry.color, picked_color);
          }
          if (!already_added) {
            context.palette.push_back({picked_color, color_to_hex(picked_color), u, v});
            context.selected_palette_index = static_cast<int>(context.palette.size()) - 1;
          }
        }
      }
    }
    ...
    for (int index = 0; index < static_cast<int>(context.palette.size()); ++index) {
      const PaletteColor& entry = context.palette[index];
      const ImVec2 pin_position(image_min.x + entry.u * (image_max.x - image_min.x),
                                image_min.y + entry.v * (image_max.y - image_min.y));
      const bool selected = context.selected_palette_index == index;
  ...
  if (ImGui::BeginTabBar("PreviewTabs")) {
    if (ImGui::BeginTabItem("Palette")) {
      if (ImGui::Button("Clear palette")) {
        ...
        context.selected_palette_index = -1;
      }
      for (int index = 0; index < static_cast<int>(context.palette.size()); ++index) {
        ImGui::PushID(index);
        const PaletteColor& entry = context.palette[index];
        ...
        ImGui::SameLine();
        if (ImGui::Selectable(entry.hex.c_str(), context.selected_palette_index == index,
                              ImGuiSelectableFlags_AllowDoubleClick)) {
          context.selected_palette_index = index;
        }
        if (context.selected_palette_index == index && ImGui::GetIO().KeyCtrl &&
